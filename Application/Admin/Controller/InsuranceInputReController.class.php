...
class InsuranceInputReController extends AdminbaseController{
	//续期首页展示
            ...
            if(!empty($data)){
               ...
               if ($k != 0){
                  ...
                  // $data [''] = $v[20];//地址
                   ...
                   $data ['surrender_date'] = $v[27];//退保日期

                   $data ['insurance_num'] = $data ['policy_number'];
                   $data ['two_time'] = $v[28];
                   $data ['two_agency_fee'] = $v[29];
                   $data ['two_state'] = ($v[30] == '成功'?1 : 2);

                   $data ['three_time'] = $v[31];
                   $data ['three_agency_fee'] = $v[32];
                   $data ['three_state'] = ($v[33] == '成功'?1 : 2);

                   $data ['four_time'] = $v[34];
                   $data ['four_agency_fee'] = $v[35];
                   $data ['four_state'] = ($v[36] == '成功'?1 : 2);

                   $data ['five_time'] = $v[37];
                   $data ['five_agency_fee'] = $v[38];
                   $data ['five_state'] = ($v[39] == '成功'?1 : 2);

                   $data ['renew_count'] = $v[40];
                   ...
                   $result1 = M('renew') -> add($data);

                   $result = M ( 'insurance_re' )->add ( $data );
                 if (!$result) 
                 {
                      $this->error ( '导入数据库失败' );
                  }
