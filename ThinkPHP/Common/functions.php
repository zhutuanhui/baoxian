...
*  作者：JBS
*/
function district($level,$pid){
		$district = M('district');
		$where['level'] = $level;
		$where['parent_id'] = $pid;
		//查询地址相关信息
		$coupon_id = $district->where($where)->field('id,name,level')->select();
		return $coupon_id;
 ...
 * 获取机构联动菜单-lzj
 *
 * @params level 门店级别 1分公司 2旗舰店 3标准店
 * @params branchid 所属分公司ID
 * */
function shopMenu($level=1,$branchid=null,$flagid=null){
    $organizationModel = M('organization');
    $where['level'] = $level;
    if(!empty($branchid)){
        $where['branch_shop_code'] = $branchid;
    }
    if(!empty($flagid)){
        $where['flagship_code'] = $flagid;
    }
    $shop_info = $organizationModel
        ->where($where)
        //所属分公司代码,所属旗舰店编码,门店编码,门店级别
        ->field('branch_shop_code,flagship_code,org_code,level,name')
        ->select();
    return $shop_info;
}
